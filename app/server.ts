...
function csvToJson(csvText: string): object[] {
...
server.tool("add-constants", "Add constants in both english and turkish languages", {
    title: z.string().min(1).max(100).describe("Title of the constant to add"),
    title__en: z.string().min(1).max(100).describe("Başlık"),
    type: z.enum(["bolum", "dil", "mevki", "saha", "ilce", "muhesebesebep", "ulke", "egitim"]).describe("Type of the constant"),
}, {
    title: "Add Constant",
    readOnlyHint: false,
    ...
    openWorldHint: true
}, async (args: { title: string; title__en: string; type: "bolum" | "dil" | "mevki" | "saha" | "ilce" | "muhesebesebep" | "ulke" | "egitim"; }, extra) => {
    try {
        const loginId = "qwer1234";
        const response = await axios.post("http://bmo.localhost/constant/actions/add.php", {
            title: args.title,
...
server.tool("get-constants", "Get constants in bmo database", {}, {
    ...
    try {
        const loginId = "qwer1234";
        const response = await axios.get("http://bmo.localhost/constant/data/csv.php", {
            headers: {
...
server.tool("search-constants", "Search constants in bmo database by query. the system can perform a search for title, title__en and type fields. The search string can appear anywhere in these fields. Type could be in English or Turkish. It is possible to restrict search to the start by placing a double quote at the start of the string, restrict to the end by placing a quote at the end or exact match by placing quotes around the entire string. Multiple search strings can be specified using a comma in between. Each string must match for a record to be displayed. type can optionally be filtered using filter_type. Filter must use the key for the type, which could be one of the following: 'bolum','ulke','mevki','saha','ilce','muhesebesebep','egitim','dil'", {
    ...
    try {
        const loginId = "qwer1234";
        const response = await axios.get(`http://bmo.localhost/constant/data/csv.php?fieldset=list&=&length=36&search=${args.query}`, {
            headers: {
                "Content-Type": "text/csv",
                "Cookie": `bmo_LOGINID=${loginId}`,
            }
            ...
            contents: [
                {
                    ...
                    text: `Error occurred: ${typeof error === "object" && error !== null
                            ? (error as any).headers?.message || (error as any).message || 'Unknown error'
                            : String(error)
                        }`,
                }
            ]
...
server.resource("constants-by-query", new ResourceTemplate("constants://by-query/{query}", { list: undefined }), {
    ...
    try {
        const loginId = "qwer1234";
        const response = await axios.get(`http://bmo.localhost/constant/data/csv.php?fieldset=list&=&length=36&search=${query}`, {
            headers: {
                "Content-Type": "text/csv",
                "Cookie": `bmo_LOGINID=${loginId}`,
            }
